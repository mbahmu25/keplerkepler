...
import { DataType, Vector, BufferType, Data, Type, Duration, Map_, FixedSizeList, FixedSizeBinary, Union, Struct, List, Interval, Timestamp, Time, Date_, Decimal, Bool, Utf8, Binary, Float, Int, Null, Field } from 'apache-arrow';
...
function forEachSegmentInPolygon(points, visitor, options = {}) {
  ...
  const isClosedEx = isClosed || equals$1(points[start], points[endPointIndex]) && equals$1(points[start + 1], points[endPointIndex + 1]);
...
function isEar(ear) {
  ...
  while (p !== a) {
    if (p.x >= x0 && p.x <= x1 && p.y >= y0 && p.y <= y1 && pointInTriangle(ax, ay, bx, by, cx, cy, p.x, p.y) && area$1(p.prev, p, p.next) >= 0) return false;
    p = p.next;
  }
...
function isEarHashed(ear, minX, minY, invSize) {
  ...
  while (p && p.z >= minZ && n && n.z <= maxZ) {
    if (p.x >= x0 && p.x <= x1 && p.y >= y0 && p.y <= y1 && p !== a && p !== c && pointInTriangle(ax, ay, bx, by, cx, cy, p.x, p.y) && area$1(p.prev, p, p.next) >= 0) return false;
    p = p.prevZ;
    if (n.x >= x0 && n.x <= x1 && n.y >= y0 && n.y <= y1 && n !== a && n !== c && pointInTriangle(ax, ay, bx, by, cx, cy, n.x, n.y) && area$1(n.prev, n, n.next) >= 0) return false;
    n = n.nextZ;
  }
...
function hardClone(data, force = false) {
    ...
    // sliced.
    return new Data(data.type, data.offset, data.length, 
...
export { index$1 as algorithm, child, data, type, vector, index as worker };
